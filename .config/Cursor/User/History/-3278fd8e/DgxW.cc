  ...
  void process_station(const string &station, size_t tick) {
    if (!manages_station(station)) {
      if (rank == 1) { // Debug output
        std::cout << "Process " << rank << " skipping unmanaged station: " 
                  << station << std::endl;
      }
      return;
    }
    ...
    if (rank == 1) { // Debug output
      std::cout << "Process " << rank << " processing station: " << station 
                << " at tick " << tick << std::endl;
    }
    ...
    for (auto &platform : station_platforms) {
        if (platform.train_id != -1) {
            ...
            if (rank == 1) {
                std::cout << "Processing train " << train.id << " at platform in " 
                          << station << ", ticks remaining: " << train.ticks_remaining 
                          << std::endl;
            }
    ...
    for (auto &link : links) {
        if (link.to_station == station && link.train_id != -1) {
            ...
            if (rank == 1) {
                std::cout << "Processing train " << train.id << " in link to " 
                          << station << ", ticks remaining: " << train.ticks_remaining 
                          << std::endl;
            }
