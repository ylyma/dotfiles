...
private:
  ...
  void process_station(const string &station, size_t tick) {
    ...
    // Process platforms
    for (auto &platform : station_platforms) {
        // Handle train at platform
        if (platform.train_id != -1) {
            Train &train = trains[platform.train_id];
            
            // Add debug logging
            if (rank == 0) {
                std::cout << "Train " << train.id << " at platform, remaining ticks: " 
                          << train.ticks_remaining << std::endl;
            }
            ...
            if (train.ticks_remaining > 0) {
                ...
                if (moved && !platform.holding_queue.empty()) {
                    // Move next train from holding to platform
                    auto next = platform.holding_queue.top();
                    platform.holding_queue.pop();
                    move_to_platform(trains[next.second], platform, tick);
                }
            }
        } else if (!platform.holding_queue.empty()) {
            // Move train from holding to empty platform
            auto next = platform.holding_queue.top();
            platform.holding_queue.pop();
            move_to_platform(trains[next.second], platform, tick);
        }
...
public:
  ...
  void run(size_t num_ticks_to_print) {
    for (size_t tick = 0; tick < total_ticks; tick++) {
        ...
        // Make sure all processes finish processing before gathering updates
        MPI_Barrier(MPI_COMM_WORLD);

        // Gather and sync updates more frequently
        sync_state();  // Add an extra sync here
