...
struct TrainStateUpdate {
    ...
    size_t ticks_remaining;
    char current_station[32];  // Fixed-size array instead of string
};
  ...
  void process_station(const string &station, size_t tick) {
    ...
    for (auto &train : trains) {
        if (train.state == Train::IN_HOLDING && train.current_station == station) {
            if (rank == 1) {
                std::cout << "Found holding train " << train.id << " at station " << station 
                         << " trying to go to " << train.next_station << std::endl;
            }

            // Try to find an empty platform going to the right destination
            for (auto &platform : station_platforms) {
                if ((size_t)platform.train_id == SIZE_MAX && platform.next_station == train.next_station) {
                    if (rank == 1) {
                        std::cout << "Moving train " << train.id << " to platform" << std::endl;
                    }
    ...
    for (auto &link : links) {
        if (link.from_station == train.current_station && 
            ...
            if (link.train_id == SIZE_MAX || link.train_id == train.id) {
                ...
                strncpy(update.current_station, link.from_station.c_str(), 31);
                update.current_station[31] = '\0';
                
                // Create MPI datatype for TrainStateUpdate
                MPI_Datatype update_type;
                int blocklengths[] = {1, 1, 1, 32};
                MPI_Aint displacements[4];
                MPI_Datatype types[] = {MPI_UNSIGNED_LONG, MPI_INT, MPI_UNSIGNED_LONG, MPI_CHAR};
                
                MPI_Get_address(&update.train_id, &displacements[0]);
                MPI_Get_address(&update.new_state, &displacements[1]);
                MPI_Get_address(&update.ticks_remaining, &displacements[2]);
                MPI_Get_address(&update.current_station, &displacements[3]);
                
                // Make relative
                MPI_Aint base;
                MPI_Get_address(&update, &base);
                for (int i = 0; i < 4; i++) {
                    displacements[i] = MPI_Aint_diff(displacements[i], base);
                }
                
                MPI_Type_create_struct(4, blocklengths, displacements, types, &update_type);
                MPI_Type_commit(&update_type);
                
                // Broadcast the update
                MPI_Bcast(&update, 1, update_type, rank, MPI_COMM_WORLD);
                ...
                train.current_station = link.from_station;
