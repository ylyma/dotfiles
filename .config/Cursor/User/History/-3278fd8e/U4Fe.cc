...
private:
  ...
  void process_station(const string &station, size_t tick) {
    ...
    if (rank == 1) {
        std::cout << "Processing station " << station << std::endl;
    }

    auto &station_platforms = platforms[station];
    for (auto &platform : station_platforms) {
        // First, check if there's a train at the platform that needs processing
        if (platform.train_id != SIZE_MAX) {
            // Find the train
            Train *train = nullptr;
            for (auto &t : trains) {
                if (t.id == platform.train_id) {
                    train = &t;
                    break;
                }
            }
            
            if (train && train->ticks_remaining == 0) {
                // Try to move train to link
                try_move_to_link(*train, platform);
        ...
        // If we get here, either platform is empty or train isn't ready to move
        // Check holding queue
        while (!platform.holding_queue.empty()) {
            ...
            if (rank == 1) {
                std::cout << "Found train " << train_id << " in holding queue at " 
                         << station << " trying to go to " << platform.next_station << std::endl;
            }
            ...
            if (platform.train_id == SIZE_MAX) {
                ...
                train->ticks_remaining = platform.load_gen.next(train_id);
                platform.holding_queue.pop();
    ...
    for (auto &link : links) {
        if (link.from_station == train.current_station && 
            ...
            if (link.train_id == SIZE_MAX || link.train_id == train.id) {
                ...
                update.new_state = train.state;
                update.ticks_remaining = train.ticks_remaining;
