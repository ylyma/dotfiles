    ...
    for (auto &link : links) {
        if (link.from_station == train.current_station && 
            ...
            if (link.train_id == SIZE_MAX || link.train_id == train.id) {
                ...
                update.current_station[31] = '\0';  // Ensure null termination
                
                // Broadcast state update to all processes
                MPI_Bcast(&update, sizeof(TrainStateUpdate), MPI_BYTE, rank, MPI_COMM_WORLD);
                
                // Apply update locally
                train.state = update.new_state;
                train.ticks_remaining = update.ticks_remaining;
                train.current_station = update.current_station;
