    ...
    for (auto &link : links) {
        if (link.from_station == train.current_station && 
            ...
            if (link.train_id == SIZE_MAX || link.train_id == train.id) {
                ...
                // Create packed buffer for MPI communication
                char buffer[MAX_BUFFER_SIZE];
                int position = 0;
                
                // Pack the data
                MPI_Pack(&train.id, 1, MPI_UNSIGNED_LONG, 
                        buffer, MAX_BUFFER_SIZE, &position, MPI_COMM_WORLD);
                
                int new_state = Train::IN_LINK;
                MPI_Pack(&new_state, 1, MPI_INT,
                        buffer, MAX_BUFFER_SIZE, &position, MPI_COMM_WORLD);
                
                MPI_Pack(&link.distance, 1, MPI_UNSIGNED_LONG,
                        buffer, MAX_BUFFER_SIZE, &position, MPI_COMM_WORLD);
                
                int station_len = link.from_station.length();
                MPI_Pack(&station_len, 1, MPI_INT,
                        buffer, MAX_BUFFER_SIZE, &position, MPI_COMM_WORLD);
                
                MPI_Pack(link.from_station.c_str(), station_len, MPI_CHAR,
                        buffer, MAX_BUFFER_SIZE, &position, MPI_COMM_WORLD);
                
                // Broadcast the packed buffer
                MPI_Bcast(&position, 1, MPI_INT, rank, MPI_COMM_WORLD);
                MPI_Bcast(buffer, position, MPI_PACKED, rank, MPI_COMM_WORLD);
