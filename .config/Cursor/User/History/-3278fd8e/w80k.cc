...
private:
  ...
  void process_station(const string &station, size_t tick) {
    if (rank == 0) {
        std::cout << "Processing station " << station << " at tick " << tick << std::endl;
    }

    auto &station_platforms = platforms[station];

    // First, try to move trains from holding to empty platforms
    for (auto &platform : station_platforms) {
        if (platform.train_id == -1 && !platform.holding_queue.empty()) {
            auto next = platform.holding_queue.top();
            platform.holding_queue.pop();
            
            if (rank == 0) {
                std::cout << "Moving train " << next.second << " from holding to platform at " 
                          << station << std::endl;
            }
            
            move_to_platform(trains[next.second], platform, tick);
        }
    }

    // Then process trains at platforms and in links
    for (auto &platform : station_platforms) {
        if (platform.train_id != -1) {
            Train &train = trains[platform.train_id];
            
            if (rank == 0) {
                std::cout << "Train " << train.id << " at platform in " << station 
                          << ", remaining ticks: " << train.ticks_remaining << std::endl;
            }

            if (train.ticks_remaining > 0) {
                train.ticks_remaining--;
            } else {
                bool moved = try_move_to_link(train, platform);
                
                if (rank == 0) {
                    std::cout << "Attempted to move train " << train.id 
                              << " to link. Success: " << moved << std::endl;
                }
            }
        }
    }
    ...
    for (auto &link : links) {
        if (link.to_station == station && link.train_id != -1) {
            Train &train = trains[link.train_id];
            
            if (rank == 0) {
                std::cout << "Train " << train.id << " in link to " << station 
                          << ", remaining ticks: " << train.ticks_remaining << std::endl;
            }
            ...
            if (train.ticks_remaining > 0) {
                ...
                handle_train_arrival(train, link, tick);
            }
        }
    }
  }
  ...
  bool try_move_to_link(Train &train, Platform &platform) {
    ...
    Link *target_link = nullptr;
    for (auto &l : links) {
        if (l.from_station == train.current_station &&
            l.to_station == train.next_station && 
            l.train_id == -1) {  // Make sure link is empty
            target_link = &l;
            break;
        }
    }
    ...
    if (!target_link) {
        if (rank == 0) {
            std::cout << "Failed to find empty link from " << train.current_station 
                      << " to " << train.next_station << std::endl;
        }
        return false;
    }
    ...
    train.ticks_remaining = target_link->distance;
    target_link->train_id = train.id;
    platform.train_id = -1;

    if (rank == 0) {
        std::cout << "Successfully moved train " << train.id << " to link" << std::endl;
    }
        ...
        if (rank == 0) {
            ...
            for (const auto &train : trains) {
                std::cout << "Train " << train.id << ": state=" << train.state 
                          ...
                          << ", next=" << train.next_station << std::endl;
            }
        }
