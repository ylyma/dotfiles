...
async function bootstrap() {
  ...
  if (process.env.APP_ENV !== 'production') {
    ...
    // OpenAPI json available at this url
    app.getHttpAdapter().get('/api-json', (req: Request, res: Response) => {
      res.json(document);
    });

    // redoc page available at this url
    const express = app.getHttpAdapter().getInstance();
    
    // Serve the Redoc standalone JS file
    express.get('/redoc-standalone.js', (req: Request, res: Response) => {
      const nodeModulesPath = path.join(process.cwd(), 'node_modules');
      
      // Possible locations for the redoc.standalone.js file
      const possiblePaths = [
        path.join(nodeModulesPath, 'redoc/bundles/redoc.standalone.js'),
        path.join(nodeModulesPath, 'redoc/dist/bundles/redoc.standalone.js')
      ];
      
      // Find first path that exists
      for (const filePath of possiblePaths) {
        if (fs.existsSync(filePath)) {
          console.log(`Serving Redoc from: ${filePath}`);
          return res.sendFile(filePath);
        }
      }
      
      // If file not found, return error
      console.error('Redoc standalone file not found. Looked in:', possiblePaths);
      return res.status(404).send('Redoc file not found. Please install the redoc package.');
    });
    
    // Serve the Redoc HTML page
    app.getHttpAdapter().get('/redoc', (req: Request, res: Response) => {
      const html = `
        <!DOCTYPE html>
        <html>
          <head>
            <title>AltaX Exchange API Documentation</title>
            <meta charset="utf-8"/>
