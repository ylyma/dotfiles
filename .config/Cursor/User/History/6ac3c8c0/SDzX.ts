...
import { ApiOperation, ApiSecurity, ApiTags, ApiResponse, ApiBadRequestResponse, ApiNotFoundResponse } from '@nestjs/swagger';
...
/**
 * Historical pricing data for trading pairs
 * 
 * Auto-generated endpoints:
 * GET /pricing-history - List pricing history with filters
 * GET /pricing-history/:id - Get single pricing record
 * POST /pricing-history - Create pricing record
 * PATCH /pricing-history/:id - Update pricing record
 */
@Crud({
  model: {
    type: PricingHistory,
...
export class PricingHistoryController implements CrudController<PricingHistory> {
  ...
  @ApiOperation({
    summary: 'Recalculate all pricing history',
    description: 'Recalculates daily, weekly and monthly pricing history for all trading pairs from given start date'
  })
  @ApiResponse({
    status: 200,
    description: 'Pricing history recalculated'
  })
  ...
  @Post('recalculate')
  @ApiOperation({
    summary: 'Recalculate specific trading pair pricing',
    description: 'Recalculates daily, weekly and monthly pricing for a trading pair from given start date'
  })
  @ApiResponse({
    status: 200,
    description: 'Pricing history recalculated'
  ...
  @Get('/graph/:id')
  @ApiOperation({
    summary: 'Get pricing graph data',
    description: 'Returns daily, weekly and monthly pricing data for graphing. Includes bid, ask and traded prices.'
  })
  @ApiResponse({
    status: 200,
    description: 'Graph data retrieved',
    type: ResponseAskGraphDto
  })
  @ApiNotFoundResponse({
    description: 'Trading pair not found or graph not enabled'
  })
